...
public class BezierCurve : MonoBehaviour {
    ...
    public Vector3 GetPointAt(float t) {
        ...
        else if (close == false && t >= 1) return points[points.Length - 1].position;
        else if (close == true && t >= 1) return points[0].position;
    ...
    private void AddCurvePoints(BezierPoint p1, BezierPoint p2, int resolution) {
        if (p1 == null || p2 == null) return;
        ...
        for (int i = 1; i < limit; i++) {
            ...
            if(curvePoints.Contains(currentPoint) == false) {
                curvePoints.Add(currentPoint);
                //DebugDraw.DrawMarker(currentPoint, 1f, Color.blue, 30f);
            }
        }
    }
